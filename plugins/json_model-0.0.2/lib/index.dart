    ...
    if (FileSystemEntity.isFileSync(f.path)) {
      ...
      if(name.contains("_")) {
        ...
        for(String s in temp) {
          className += s[0].toUpperCase() + s.substring(1);
        }
      } else {
...
String changeFirstChar(String str, [bool upper=true] ){
  ...
  return upper?className:str[0].toLowerCase()+str.substring(1);
}
...
String getType(v,Set<String> set,String current, tag){
  ...
  if(v is bool){
    ...
    return "List";
  }else if(v is String){ //处理特殊标志 注tag=$
    if(v.startsWith("$tag[]")){
